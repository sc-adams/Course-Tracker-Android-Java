...
public class DBHelper extends SQLiteOpenHelper {
    ...
    // TODO: implement binary search tree for loading and sorting before adding to the db
    public void loadCourses(SQLiteDatabase db) {
        ...
        try {
            ...
            String filePath = "courses.txt";  // the exact location of the file is: $(ProjectDir)\app\src\
            // asset manager will automatically duplicate the file to : $(ProjectDir)\app\src\main\assets

            reader = new BufferedReader(new InputStreamReader(assetManager.open(filePath)));

            String line;
            while ((line = reader.readLine()) != null) {   // if not null
                String[] bidData = line.split(",");  // split the values with the comma
                if (bidData.length >= 2) {  // if there are 2 or more than 2 comma separated values
                    String title = bidData[0];  // the first is the title
                    String description = bidData[1];  // the second is the description
                    List<String> prerequisites = new ArrayList<>();
                    if (bidData.length > 2) {  // if there are more than 2 comma separated values
                        for (int i = 2; i < bidData.length; i++) {
                            prerequisites.add(bidData[i]);  // all that follow are added to the prerequisites array
                        }
                    }
                    cv.put(COLUMN_TITLE, title);  // add title to the db
                    cv.put(COLUMN_DESCRIPTION, description);  // add description to the db
                    cv.put(COLUMN_PREREQUISITES, prerequisites.toString());  // add the prerequisites list cast to string to the db
                    cv.put(COLUMN_PERCENT, percent);  // add the precalculated percent to the db
                    long result = db.insert(COURSE_TABLE, null, cv);  // get the result of the insertion
                    if (result != -1) {  // successfully added row
        ...
        } catch (IOException e) {  // catch the exceptions
            e.printStackTrace();
