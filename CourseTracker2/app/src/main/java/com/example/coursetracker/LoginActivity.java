...
public class LoginActivity extends AppCompatActivity {
    ...
    public void addCredentials(String enteredUsername){
        ...
        // check if a phone is entered, add it if it is valid
        enteredPhone = phone_input.getText().toString().trim();
        if (enteredPhone != null){  // first check for null
            if (isValidPhone(enteredPhone)) {  // check if the field is valid
                if (!enteredPhone.isEmpty()){  // if the field is not empty and it is valid, set the phone
